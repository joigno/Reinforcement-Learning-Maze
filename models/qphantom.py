...
from models.phantom import PhantomMemory


class QPhantomModel(AbstractModel):
    ...
    def train(self, stop_at_convergence=False, **kwargs):
        ...
        for episode in range(1, episodes + 1):
            ...
            status = phantom_status = start_status
            start_list.remove(start_cell)
            #self.phantom_memory.reset_excitability()
            ...
            while True:
                ...
                if np.random.random() < exploration_rate:
                    ...
                    action = self.predict(state,phantom_status)

                current_status = status
                next_state, reward, status = self.environment.step(action)

                # check phantom memory
                phantom_status = self.phantom_memory.recall(state,current_status,action,status,reward)

                next_state = tuple(next_state.flatten()+[Status.status_to_int(phantom_status)])
